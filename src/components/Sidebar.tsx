...
const Sidebar: React.FC<SidebarProps> = ({ className }) => {
  ...
  return (
    <div className={cn("w-64 h-screen fixed left-0 top-0 navy-bg text-white", className)}>
      <div className="flex items-center p-4 border-b border-slate-700">
        <div className="font-bold text-xl">
          <span className="text-white mr-1">JD</span>
          <span className="text-purple-light">|</span>
          <span className="text-purple-light ml-1">Modern Solutions</span>
        </div>
      </div>

      <div className="p-4 border-b border-slate-700">
        <p className="text-gray-300">Welcome,</p>
        <h3 className="text-xl font-medium">{user?.username || 'Guest'}</h3>
